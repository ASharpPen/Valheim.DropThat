...
internal class ModifierMagicItem : IDropModifier
{
    ...
    public void Modify(DropContext context)
    {
        ...
        if (context.Extended.Config.TryGet(CharacterDropModConfigEpicLoot.ModName, out var cfg) && cfg is CharacterDropModConfigEpicLoot modConfig)
        {
            config = modConfig;
        }
        else
    ...
    {
        var uniqueIds = config.UniqueIDs.Value;
        ...
        if (uniqueIds.Count > 0)
        {
            ...
            if (UniqueLegendaryHelper.TryGetLegendaryInfo(randomId, out LegendaryInfo legendaryInfo))
            {
                ...
                {
                    Log.LogWarning($"Attempted to roll Epic Loot unique legendary with id '{randomId}' for Drop That config entry '{config.SectionPath}' but requirements were not met. Skipping.");
                    return;
                }
