...
internal class ConditionBossAffix : ICondition
{
    ...
    public static ConditionBossAffix Instance => _instance ??= new ConditionBossAffix();
    ...
    public bool ShouldFilter(CharacterDrop.Drop drop, DropExtended extended, CharacterDrop characterDrop)
    {
        ...
        if (extended.Config.Subsections.TryGetValue(CharacterDropModConfigCLLC.ModName, out var config) && config is CharacterDropModConfigCLLC cllcConfig)
        {
    ...
    {
        if (config.ConditionBossAffix.Value.Count > 0)
        {
            var states = config.ConditionBossAffix.Value;
    ...
    {
        if (config.ConditionNotBossAffix.Value.Count > 0)
        {
            var states = config.ConditionNotBossAffix.Value;
