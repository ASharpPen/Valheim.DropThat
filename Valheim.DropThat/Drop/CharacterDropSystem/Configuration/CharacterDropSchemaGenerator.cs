...
internal static class CharacterDropSchemaGenerator
{
    internal static TomlSchemaBuilder GenerateCfgSchema()
    {
        ...
        var dropConfigBuilder = builder
            ...
            .GetNode()
            .AddSetting<List<string>>("UseDropList", null, "List of droplists to load before other drop settings.")
    ...
    {
        MappingInstantiationForParent<CharacterDropSystemConfiguration, CharacterDropBuilder> mobMappings = new()
        {
            SubPath = new()
            {
            ...
            Instantiation = new()
            {
                Instantiation = (CharacterDropSystemConfiguration parent, TomlConfig config) => parent.GetBuilder(config.PathSegment.Name),
            },
            Action = (CharacterDropSystemConfiguration parent, CharacterDropBuilder instance, TomlConfig config) =>
            {
                if (config.Settings.TryGetValue("UseDropList", out var tomlSetting) &&
                    tomlSetting is ITomlSetting<List<string>> setting &&
                    setting.IsSet)
                {
                    instance.ListNames = setting.Value;
                }
            },
            SubInstantiations = new()
            {
                new MappingInstantiationForParent<CharacterDropBuilder, CharacterDropDropBuilder>()
                {
