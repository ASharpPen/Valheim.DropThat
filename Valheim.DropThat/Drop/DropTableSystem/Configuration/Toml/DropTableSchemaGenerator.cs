...
internal static class DropTableSchemaGenerator
{
    internal static TomlSchemaBuilder GenerateCfgSchema()
    {
        var builder = new TomlSchemaBuilder();

        var dropTableBuilder = builder
            .SetLayerAsCollection() // Table layer
            .GetNode()
            .AddSetting<int?>("SetDropMin", 1, "Minimum of randomly selected entries from drop list. Entries can be picked more than once.")
            .AddSetting<int?>("SetDropMax", 1, "Maximum of randomly selected entries from drop list. Entries can be picked more than once.")
            .AddSetting<float?>("SetDropChance", 100, "Chance to drop anything at all.")
            .AddSetting<bool?>("SetDropOnlyOnce", false, "If true, will ensure that when selecting entries from drop list, same entry will only be picked once.")
            .AddSetting<List<string>>("UseDropList", default, "List of droplists to load before other drop settings.");
            ;

        var dropLayer = dropTableBuilder
    ...
    internal static ConfigToObjectMapper<DropTableSystemConfiguration> GenerateConfigLoader(DropTableSystemConfiguration configuration)
    {
        return new ConfigToObjectMapper<DropTableSystemConfiguration>()
        {
            ...
            {
                new MappingInstantiationForParent<DropTableSystemConfiguration, DropTableBuilder>()
                {
                    ...
                    Instantiation = new()
                    {
                        Instantiation = (DropTableSystemConfiguration parent, TomlConfig config) =>
                            parent.GetBuilder(config.PathSegment.Name),
                        InstanceActions = new()
                        {
                    ...
                    SubInstantiations = new()
                    {
                        new MappingInstantiationForParent<DropTableBuilder, DropTableDropBuilder>()
                        {
                            SubPath = new(){ new TomlPathSegment(TomlPathSegmentType.Collection)},
                            Instantiation = new()
                            { 
                                Instantiation = (DropTableBuilder parent, TomlConfig config) =>
